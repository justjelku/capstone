...
define("ace/mode/vbscript_highlight_rules", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text_highlight_rules"], function (e, t, n) {
  "use strict";
...
}), define("ace/mode/folding/vbscript", ["require", "exports", "module", "ace/lib/oop", "ace/mode/folding/fold_mode", "ace/range", "ace/token_iterator"], function (e, t, n) {
  "use strict";
  ...
  r.inherits(u, i), function () {
    this.indentKeywords = {
      ...
      next: -1,
      wend: -1
    }, this.foldingStartMarker = /(?:\s|^)(class|function|sub|if|select|do|for|while|with|property|else|elseif)\b/i, this.foldingStopMarker = /\b(end|loop|next|wend)\b/i, this.getFoldWidgetRange = function (e, t, n) {
      var r = e.getLine(n),
          i = this.foldingStartMarker.test(r),
          s = this.foldingStopMarker.test(r);

      if (i || s) {
        var o = s ? this.foldingStopMarker.exec(r) : this.foldingStartMarker.exec(r),
            u = o && o[1].toLowerCase();

        if (u) {
          var a = e.getTokenAt(n, o.index + 2).type;
          if (a === "keyword.control.asp" || a === "storage.type.function.asp") return this.vbsBlock(e, n, o.index + 2);
        }
      }
      ...
      if (i && !s) {
        ...
        if (u) {
          var a = e.getTokenAt(n, o.index + 2).type;
          if (a == "keyword.control.asp" || a == "storage.type.function.asp") return u == "if" && !/then\s*('|$)/i.test(r) ? "" : "start";
        }
      }
      ...
      switch (l) {
        ...
        case "with":
          ...
          var m = new RegExp("(?:^|\\s)" + l, "i"),
              g = /^\s*End\s(If|Sub|Select|Function|Class|With|Property)\s*/i.test(d);
      ...
      while (a = i.step()) {
        ...
        if (C > 0) c.unshift(l);else if (C <= 0 && N === !1) {
          c.shift();
          ...
          if (!c.length) {
            switch (l) {
              case "end":
                var y = i.getCurrentTokenPosition();
                T = i.getCurrentTokenRange(), i.step(), i.step(), a = i.getCurrentToken();

                if (a) {
                  l = a.value.toLowerCase();

                  if (l in u) {
                    f == "else" || f == "elseif" ? l !== "if" && x.shift() : l != f && x.shift();
                    var b = i.getCurrentTokenPosition(),
                        w = b.column + l.length;
...
}), define("ace/mode/vbscript", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/mode/vbscript_highlight_rules", "ace/mode/folding/vbscript", "ace/range"], function (e, t, n) {
  "use strict";
  ...
  r.inherits(a, i), function () {
    function t(e, t, n) {
      ...
      for (var i = 0; i < e.length; i++) {
        ...
        if (s.type == "keyword.control.asp" || s.type == "storage.type.function.asp") {
