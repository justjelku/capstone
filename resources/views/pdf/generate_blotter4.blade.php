...
<body>
    <div class="container">
        ...
        <div class="content paragraph-container">
            ...
            <p style="margin-bottom: 2px;">4th QUARTER CY 2023</p>
        </div>
...
<tbody>
    ...
    <tr>
        ...
        {{-- <td style="padding-bottom: 90px; text-align:center">{{ $blotter->where('nature_cases', 'civil')->count() }}</td>
        <td style="padding-bottom: 90px; text-align:center">{{ $blotter->where('nature_cases', 'others')->count() }}</td>
        <td style="padding-bottom: 90px; text-align:center">
            {{ 
                $blotter->where('nature_cases', 'criminal')->count() + 
                $blotter->where('nature_cases', 'civil')->count() +
                $blotter->where('nature_cases', 'others')->count() 
            }}
        </td>
        <td style="padding-bottom: 90px; text-align:center">{{ $blotter->where('settled_cases', 'mediation')->count() }}</td>
        <td style="padding-bottom: 90px; text-align:center">{{ $blotter->where('settled_cases', 'conciliated')->count() }}</td>
        <td style="padding-bottom: 90px; text-align:center">{{ $blotter->where('settled_cases', 'arbitrution')->count() }}</td>
        <td style="padding-bottom: 90px; text-align:center">
            {{ 
                $blotter->where('settled_cases', 'mediation')->count() + 
                $blotter->where('settled_cases', 'conciliated')->count() +
                $blotter->where('settled_cases', 'arbitrution')->count() 
            }}
        </td>
        <td style="padding-bottom: 90px; text-align:center">{{ $blotter->where('action_taken', 'repudiated')->count() }}</td>
        <td style="padding-bottom: 90px; text-align:center">{{ $blotter->where('action_taken', 'dismissed')->count() }}</td>
        <td style="padding-bottom: 90px; text-align:center">{{ $blotter->where('action_taken', 'referred')->count() }}</td>
        <td style="padding-bottom: 90px; text-align:center">{{ $blotter->where('action_taken', 'certified')->count() }}</td>
        <td style="padding-bottom: 90px; text-align:center">{{ $blotter->where('action_taken', 'pending')->count() }}</td>
        <td style="padding-bottom: 90px; text-align:center">
            {{ 
               $blotter->where('action_taken', 'repudiated')->count() +
               $blotter->where('action_taken', 'dismissed')->count() +
               $blotter->where('action_taken', 'referred')->count() +
               $blotter->where('action_taken', 'certified')->count() +
               $blotter->where('action_taken', 'pending')->count() 
